...
This code performs data quality checks on the `drug_inventory_management` table and generates a DataFrame with the results of each check. It includes checks for mandatory fields, expiry date validation, uniqueness, and data consistency. Each check calculates the pass percentage and determines if the check passed or failed.