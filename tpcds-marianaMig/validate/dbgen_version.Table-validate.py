...
with redshiftEngine.connect() as connection:
	result = connection.execute("SELECT * FROM information_schema.columns WHERE table_schema = 'tpcds' AND table_name = 'dbgen_version'")
	for row in result:
...
redshiftCount = 0
with redshiftEngine.connect() as connection:
	result = connection.execute("select count(*) as Count from tpcds.dbgen_version")
	for row in result:
...
databricksCols = list()
tablename = databricksCatalog + "." + databricksSchema + "." + "dbgen_version"
