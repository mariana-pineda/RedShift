...
databricksCols = list()
tablename = databricksCatalog + "." + databricksSchema + "." + "category"
...
# Check each column
charColValidate = """select sum((COLNAME IS NULL)::int) as null, round(avg(cast(len(COLNAME) as float)), 2) as avg, min(len(COLNAME)) as min, max(len(COLNAME)) as max, round(stddev(len(COLNAME)), 2) as stddev from """
